...
function ordenarRamosPorProximidade(anteriores: readonly Complex[], atuais: Complex[]): Complex[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < n; j++) {
      ...
      const d = Math.hypot(anteriores[i].re - atuais[j].re, anteriores[i].im - atuais[j].im);
      if (d < menorDist) {
        menorDist = d;
        melhor = j;
      }
...
export function calcularRamosLgr(numerador: readonly number[], denominador: readonly number[], ganhoMaximo?: number): { Ks: number[]; ramos: Complex[][] } {
  ...
  if (ganhoMaximo === undefined) {
    ...
    for (const candidato of [100, 500, 1000, 5000]) {
      const poli = somarPolinomios(denominador, preencherZerosEsquerda(numerador, denominador.length).map((v) => v * candidato));
  ...
  const faixaAlta = Array.from({ length: 600 }, (_, i) => 0.5 * Math.pow(teto / 0.5, i / 599));
  const ganhos = [...new Set([...faixaBaixa, ...faixaAlta])].sort((a, b) => a - b);
  const ramos: Complex[][] = [];
  let anteriores: Complex[] | null = null;
  for (const ganho of ganhos) {
    const poli = somarPolinomios(denominador, preencherZerosEsquerda(numerador, denominador.length).map((v) => v * ganho));
    let raizesAtuais = encontrarRaizes(poli);
    while (raizesAtuais.length < numeroPolos) raizesAtuais.push(criarComplexo(NaN, NaN));
    raizesAtuais = raizesAtuais.slice(0, numeroPolos);
    if (anteriores) raizesAtuais = ordenarRamosPorProximidade(anteriores, raizesAtuais);
