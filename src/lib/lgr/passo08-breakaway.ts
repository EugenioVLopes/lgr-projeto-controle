...
import { criarComplexo, dividirComplexos, moduloComplexo, ehNumeroReal } from "./complexos";
import { multiplicarPolinomios, subtrairPolinomios, derivarPolinomio, avaliarPolinomioComplexo } from "./polinomios";
import { encontrarRaizes } from "./raizes";

export function encontrarPontosBreakaway(numerador: readonly number[], denominador: readonly number[], polos: readonly Complex[], zeros: readonly Complex[]): BreakPoint[] {
  ...
  const partesReaisPolosZeros = [...polos, ...zeros].filter((p) => ehNumeroReal(p, 1e-8)).map((p) => p.re);
  ...
  for (const candidata of candidatas) {
    ...
    const ganhoK = dividirComplexos({ re: -valorDenominador.re, im: -valorDenominador.im }, valorNumerador);
    if (Math.abs(candidata.im) < 1e-4) {
      const parteReal = candidata.re;
      const contagemADireita = partesReaisPolosZeros.filter((x) => x > parteReal + 1e-10).length;
      if (contagemADireita % 2 === 1 && ganhoK.re > 0 && Math.abs(ganhoK.im) < 1e-3)
