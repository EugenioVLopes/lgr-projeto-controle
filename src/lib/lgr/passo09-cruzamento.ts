...
import { multiplicarPolinomios, subtrairPolinomios, preencherZerosEsquerda, avaliarPolinomioReal } from "./polinomios";
import { encontrarRaizes } from "./raizes";

function separarParteRealImaginariaJw(coeficientes: readonly number[]): { parteReal: number[]; parteImaginaria: number[] } {
  ...
  for (let k = 0; k < coeficientes.length; k++) {
    ...
    if (resto === 0) mapaReal.set(potencia, (mapaReal.get(potencia) ?? 0) + coef);
    else if (resto === 1) mapaImag.set(potencia, (mapaImag.get(potencia) ?? 0) + coef);
    else if (resto === 2) mapaReal.set(potencia, (mapaReal.get(potencia) ?? 0) - coef);
...
// ---------- Passo 9a ----------
export function encontrarCruzamentosEixoImaginario(denominador: readonly number[], numerador: readonly number[]): { cruzs: Cruzamento[]; info: Record<string, number[]> } {
  const parteRealDenominador = separarParteRealImaginariaJw(denominador).parteReal;
  const parteImaginariaDenominador = separarParteRealImaginariaJw(denominador).parteImaginaria;
  const parteRealNumerador = separarParteRealImaginariaJw(numerador).parteReal;
  const parteImaginariaNumerador = separarParteRealImaginariaJw(numerador).parteImaginaria;
  ...
  let polinomioCruzamento = subtrairPolinomios(multiplicarPolinomios(Re_D, Im_N), multiplicarPolinomios(Im_D, Re_N));
  while (polinomioCruzamento.length > 1 && Math.abs(polinomioCruzamento[0]) < 1e-12) polinomioCruzamento = polinomioCruzamento.slice(1);
  ...
  for (const candidata of frequenciasCandidatas) {
    ...
    if (ganhoK > 1e-10 && !saidas.some((o) => Math.abs(o.K - ganhoK) < 1e-4 && Math.abs(o.w - omega) < 1e-4))
...
// ---------- Passo 9b ----------
export function montarTabelaRouth(denominador: readonly number[], numerador: readonly number[], ganhoK: number): number[][] {
  ...
  const coefs = preencherZerosEsquerda(denominador, tamanho).map((d, i) => d + ganhoK * numeradorAlinhado[i]);
  const grau = coefs.length - 1;
  const colunas = Math.floor((grau + 2) / 2);
  const tabela: number[][] = Array.from({ length: grau + 1 }, () => new Array(colunas).fill(0));
  for (let j = 0; j < colunas; j++) {
    if (2 * j < coefs.length) tabela[0][j] = coefs[2 * j];
    if (2 * j + 1 < coefs.length && tabela.length > 1) tabela[1][j] = coefs[2 * j + 1];
  }
  for (let i = 2; i <= grau; i++) {
    const pivo = tabela[i - 1][0];
    if (Math.abs(pivo) < 1e-12) break;
    for (let j = 0; j < colunas - 1; j++) {
      tabela[i][j] = (tabela[i - 1][0] * tabela[i - 2][j + 1] - tabela[i - 2][0] * tabela[i - 1][j + 1]) / pivo;
    }
  }
  return tabela;
}
