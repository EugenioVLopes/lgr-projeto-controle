...
export default function LgrPlot({ traces, title, descritoPor, tema }: { traces: Trace[]; title: string; descritoPor?: string; tema?: 'light' | 'dark' }) {
  const ref = useRef<HTMLDivElement>(null)
  const [carregando, setCarregando] = useState(true)
  const [erro, setErro] = useState<string | null>(null)
  const [ehMobile, setEhMobile] = useState(() => (typeof window !== 'undefined' ? window.innerWidth < 560 : false))
  ...
  useEffect(() => {
    ...
    carregarPlotly()
      .then((Plotly) => {
        ...
        Plotly.react(
          ...
          atual as never,
          {
            ...
            font: { color: ink },
            xaxis: { title: { text: 'Real' }, zeroline: true, zerolinecolor: border, gridcolor: grid, tickfont: { color: muted } },
            yaxis: { title: { text: 'Imag (jω)' }, zeroline: true, zerolinecolor: border, gridcolor: grid, tickfont: { color: muted }, scaleanchor: 'x' },
            margin: { l: 45, r: 15, t: 40, b: 40 },
            showlegend: true,
            legend: { orientation: ehMobile ? 'v' : 'h' },
          } as never,
          { responsive: true, displaylogo: false, displayModeBar: true, scrollZoom: true, useResizeHandler: true, toImageButtonOptions: { format: 'png' } },
        )
      ...
      .catch(() => {
        if (montado) {
          setCarregando(false)
          setErro('Gráfico indisponível offline. Recalcule para tentar de novo.')
        }
      })
    return () => {
      montado = false
      if (el && plotlyPromise) {
        ...
        carregarPlotly().then((Plotly) => {
          try { (Plotly as { purge?: (el: HTMLElement) => void }).purge?.(alvo) } catch { /* noop */ }
        })
      }
    }
  ...
  return (
    ...
    >
      ...
      {erro && <p className="plot-carregando" role="status">{erro}</p>}
    </div>
  )
}
